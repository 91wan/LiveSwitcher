# LiveSwitcher v0.3.0 Live Safety Cockpit

Use the Safety Cockpit before a live run when the operator needs a larger, focused readiness view instead of the compact `? -> Preflight` popover.

开演前如果需要比 `? -> Preflight` 更大的现场检查视图，使用 Safety Cockpit / 现场安全台。
