...
Program Activation planning remains a ViewModel-owned call site. When
`.programSelection` is owned, the planner input `currentItem` comes from
`runtime.state.program.effectiveCurrentItem`; when `.programQueue` is owned,
the planner input `queuedItems` comes from `runtime.state.program.items`.
`switchToProgram(_:)` canonicalizes the requested item from the Runtime queue
when an item with the same ID exists there. Source validation remains
ViewModel-owned and runs on that canonical activation item. Production bridge
mode remains `.panicOwned`.

Automation command execution effect emission is routed through
`AutomationCommandRuntimeReducer.swift`. Main `LiveRuntimeReducer.swift` routes
`automationScriptRequested` only and keeps the `.automationCommand` ownership
guard. `automationScriptRequested` requires `.automationCommand` ownership,
does not dispatch audio-input sync, and emits only the existing `runAppleScript`
effect. Recorded `runAppleScript` effects redact script source. Automation
failure / notice lifecycle remains in `AutomationNoticeRuntimeReducer`.
ViewModel owns AppleScript source construction, WPS fallback branching, and
PPT/WPS key forwarding. Production bridge mode remains `.panicOwned`.

Runtime action surface should not include no-op lifecycle actions. BGM
preparation is represented by the `.prepareBGM` effect and real playback
callbacks, not by a dead `bgmPrepared` action. Panic delayed BGM pause is
represented by `panicBGMPauseDelayElapsed`; there is no separate
`panicFadeCompleted` action. Main `LiveRuntimeReducer.swift` remains
route-only, keeping ownership guards and delegating meaningful domain behavior
...
media callback actions. When `.programSelection` is owned, current program
media-source status is derived from Runtime current program state. Audio routing
context must use Runtime-owned media, current program, Panic, and BGM state
whenever those domains are owned. When `.projection` is owned, projection
snapshot state is preserved from Runtime, not the ViewModel facade. External
display polling enters Runtime through projection callback actions, and
`makeRuntimeStateSnapshot()` must not overwrite Runtime-owned
`ProjectionRuntimeState`. `syncProjectionFacadeFromRuntime()` mirrors the
Runtime-owned `hasExternalDisplay`, `isBroadcasting`, and `safetyNotice` fields
back to the ViewModel facade. When `.bgm` is owned, BGM playback state,
progress, duration, generation, and `playMode` are preserved from
`runtime.state.bgm`; `bgmPlayMode` is a runtime-to-facade projection, not a
...
ownership. Explicit operator preference actions remain the only path for
updating Runtime-owned persisted settings.
When `.audio` is owned, `makeRuntimeStateSnapshot()` preserves Runtime-owned
audio controls and `audioFacadeSnapshot()` uses Runtime-owned audio controls.
ViewModel audio properties are facade inputs, not a second source of truth
after audio ownership. Explicit audio operator actions remain the path for
...
`ViewModel+ProgramActivation.swift`. That call site follows Runtime ownership
for planner inputs: Runtime current program supplies `currentItem` when
`.programSelection` is owned, Runtime program queue supplies `queuedItems`
when `.programQueue` is owned, and requested queued items are canonicalized
from the Runtime queue before source validation and plan construction.
`ProgramActivationPlan` explicitly models
...
and source availability classification remain ViewModel-owned and run on the
canonical activation item; classification lives in the pure
...
| Automation command execution | Runtime owner | Fire-and-forget AppleScript command request action and `runAppleScript` effect | authoritative | Runtime owns `.automationScriptRequested` and emits `.runAppleScript` only in `.automationCommandOwned`, `.presentationQueryOwned`, `.programQueueOwned`, `.programSelectionOwned`, or `.fullRuntime`. Automation command effect emission is routed through `Runtime/AutomationCommandRuntimeReducer.swift`; `LiveRuntimeReducer.swift` routes the request only and keeps the ownership guard. `automationScriptRequested` does not dispatch audio-input sync, and recorded `runAppleScript` effects redact script source. The `automation` port means fire-and-forget command execution only. ViewModel owns AppleScript source construction, concrete `AppleScriptRunner.run`, failure-to-support generation, and failure notice dispatch through `ViewModel+PresentationAutomation.swift` and `ViewModel+AutomationFailure.swift`. WPS fallback branching, PPT/WPS key forwarding, permission modal alerts, telemetry, and Support event generation decisions remain ViewModel-owned. |
...
| Preferences / persisted settings | Runtime state guarded by Persistence/Image Assets infrastructure ownership | Console mode, theme, wallpaper/logo URL, auto-play, auto-advance, agenda timeline, and corner-logo position mutation | reducer extracted, no standalone domain | Preference mutation logic lives in `Runtime/PreferencesRuntimeReducer.swift`; `LiveRuntimeReducer.swift` routes preference actions only. Persistence-backed settings require `.persistence`; wallpaper/corner-logo URL updates require both `.persistence` and `.imageAssets`. When `.persistence` is owned, `makeRuntimeStateSnapshot()` preserves `runtime.state.mode` and `runtime.state.preferences`, so stale ViewModel preference facade values cannot overwrite Runtime-owned persisted settings. Explicit operator preference actions still update Runtime-owned preferences. Preference actions do not dispatch audio-input sync. No `.preferences` domain, bridge mode, or port exists. Production bridge mode remains `.panicOwned`. |
...
When `.bgm` is owned, `makeRuntimeStateSnapshot()` preserves Runtime
`state.bgm` playback fields, including current item, playing state, progress,
current time, duration, generation, and `playMode`; only `items` are refreshed
from `runtimeBGMItemsForSnapshot()`. Reached-end decisions use Runtime
`state.bgm.playMode`, and `syncBGMFacadeFromRuntime()` projects Runtime
`playMode` back to `bgmPlayMode` for the ViewModel facade.
...
decisions, and `startPPTEventTap` / `stopPPTEventTap` effects. Those state
transitions live in `PPTRuntimeReducer`, while `LiveRuntimeReducer` only routes
PPT actions after `.ppt` ownership is active. The UI facade
`isPageInterceptEnabled` is a projection from `state.ppt.isRequested ||
state.ppt.isEventTapActive`, so requested=true / active=false is a valid
pending start state and projects as enabled. Failed or stopped states clear both
requested and active, and project the facade as disabled.
...
Automation command domain. Automation command execution effect emission is
routed through `AutomationCommandRuntimeReducer.swift`; `LiveRuntimeReducer`
routes `automationScriptRequested` only and retains the `.automationCommand`
ownership guard. `automationScriptRequested` does not dispatch audio-input sync,
and recorded `runAppleScript` effects redact script source. Production wires the
`automation` port for command execution in `.programActivationOwned`.
...
`SupportEventPort`.
Support mutation mechanics live in `Runtime/SupportRuntimeReducer.swift`; the
main `LiveRuntimeReducer.swift` routes support actions only.
`.supportEventRecorded` requires `.support` ownership before mutating state and
does not dispatch audio-input sync.
