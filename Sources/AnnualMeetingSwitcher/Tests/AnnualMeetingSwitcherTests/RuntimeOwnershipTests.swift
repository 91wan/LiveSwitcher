...
final class RuntimeOwnershipTests: XCTestCase {
    ...
    func testDocumentDoesNotClaimRuntimeAuthorityBeforeEffectsAreFullyWired() throws {
        ...
        XCTAssertTrue(normalizedDocument.localizedStandardContains("`facadeCurrentProgramChanged` is compatibility-only and handled by `ProgramSelectionRuntimeReducer`"))
        XCTAssertTrue(normalizedDocument.localizedStandardContains("Production ViewModel selection paths must not call `facadeCurrentProgramChanged`"))
        ...
        XCTAssertTrue(document.localizedStandardContains("BGM callbacks require `.bgm` ownership"))
        XCTAssertTrue(normalizedDocument.localizedStandardContains("`facadeCurrentProgramChanged` is compatibility-only and requires `.programSelection` ownership"))
