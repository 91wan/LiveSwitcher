...
enum HelpCopyModel {
    static let sections: [HelpCopySection] = [
        HelpCopySection(title: "常用必备功能（红色 = 直播 / 危险状态）", items: [
            "主持人模式：顶部按钮集中控制；开启后进入警告状态，媒体声道和 BGM 压至 7% 突出人声",
            "PPT翻页笔：顶部按钮集中控制；开启后翻页笔方向键控制 WPS/PPT 翻页（需辅助功能权限，开启失败时 App 会自动引导设置）",
            "投射副屏：右侧现场控制的输出卡片控制；直播为红色状态，无外接屏时不会投射，副屏断开会显示副屏丢失",
            "紧急切黑：顶部【紧急切黑】会让副屏黑屏、静音、暂停当前媒体并暂停 BGM，激活时为红色危险状态，再点恢复"
        ]),
        HelpCopySection(title: "视频与音频操作", items: [
            ...
            "背景音乐：右侧现场控制的 BGM 卡片提供当前曲目、基础播控、分类和少量常用曲目；完整 BGM 库位于音频页面",
